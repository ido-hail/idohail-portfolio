...
- **Status:** DONE - `public/favicon.svg` (64 viewBox, rounded dark tile, white `IH` drawn as four plain rects, no font dependency, no gradient, script, `foreignObject` or remote reference), a real multi-size `public/favicon.ico` (type 1, entries at 16, 32 and 48) and a square 180x180 `public/apple-touch-icon.png`. Wired in `BaseLayout` as ICO with `sizes="32x32"`, SVG preferred where supported, and the Apple touch icon. All three are local and resolve under `img-src 'self'`. **No web manifest**, per the Definition of Done and the served `manifest-src 'none'`. Legibility checked at 16 and 32 px: three stems and the crossbar stay distinct. The mark appears in browser identity only, never in page UI.
