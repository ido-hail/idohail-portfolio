...
- **Superseded (2026-09-14):** this task shipped as recorded above and its history stands. It has since been **reversed by a deliberate product decision**: resumes are adapted per application and maintained outside this repository rather than published as one canonical download. `public/Ido-Hail-Resume.pdf` was deleted and `siteConfig.resumePath` set back to `null`. See the `Remove public Resume integration` PR and `SPEC.md` §5. The nullable field and every conditional consumer were kept, so re-publishing needs only `resumePath` to be set again.
...
- **Superseded (2026-09-14):** the verification above was valid when it ran. With `P4-04` reversed, the same conditionals now correctly render **no** Resume surface anywhere, again with no template change — which re-proves the Phase 1 conditional in the opposite direction.
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-15 | **Skills and Projects aligned to the current resume.** The manually edited `Ido-Hail-Resume-Master-v2-short` files are the evidence baseline; the site stays a curated presentation, not keyword parity. Skill Map: eighth family `AI & Engineering Productivity` added; `Axiom` added to Production; `Helm` and `OIDC` to Cloud; `Data Analysis` added and `Data Quality` renamed `Data Quality & Validation`; `Workflow Automation` added to Programming; `Zendesk` renamed `Zendesk Administration`; `User Lifecycle`, `Access & Security Policies`, `Endpoint & Agent Administration` and `MCSA` added to Systems/Identity; `QA & Test Coordination` replaced by `QA & Test Planning`, `Integration & Regression Testing` and `E2E Testing`; the near-duplicate `Release Management`/`Release Readiness` pair merged into one chip. Projects: fourth case study `LLM Data Enrichment Pipeline` added (sanitized, no source named, no repository invented) and featured; Pac-Man promoted to featured; Portfolio Website unfeatured but retained; order is NameGen / Pac-Man / LLM / Portfolio. Tools marquee, Hero, positioning, `workAreas`, Experience, About, Resume integration and LinkedIn unchanged. |
| 2026-09-14 | **Public Resume integration removed by product decision.** Resume documents are now adapted per application and maintained outside this repository rather than presented as one canonical public download. `public/Ido-Hail-Resume.pdf` deleted; `siteConfig.resumePath` set to `null`; `SPEC.md` §5, §3.1, §3.2, §8.3 and §18.12 updated to describe the intentionally-unset state. `P4-04` and `P4-05` are marked **Superseded**, not undone — their history is preserved. The nullable `resumePath` architecture and every conditional consumer (Hero, Header, closing Contact/CTA, Footer) are kept intact, so re-enabling is a one-line configuration change. No Skills, Projects, Experience, About, positioning or LinkedIn content was touched.                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
